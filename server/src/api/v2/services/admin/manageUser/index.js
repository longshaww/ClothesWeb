...
class ManageUserService {
    ...
    async editUser(model) {
        try {
            const user = await UserWeb.findOneWithDeleted({ _id: this._idUser });
    ...
    async getIdUser() {
        try {
            const data = await UserWeb.findOneWithDeleted({ _id: ObjectId(this._idUser) });
            return data ?? null;
        } catch (err) {
            throw new Error(err.message);
        }
    ...
    async changePassword(password) {
        try {
            ...
            await UserWeb.updateOneWithDeleted(
